...
# Función para realizar la predicción usando GPU
def prediccion_gpu(modelo, entrada):
    # Convertir entrada a matriz numpy
    entrada_np = np.array(entrada).astype(np.float32)
    entrada_gpu = gpuarray.to_gpu(entrada_np)
    prediction_gpu = modelo.predict(entrada_gpu)
    prediction = prediction_gpu.get()
    return prediction
